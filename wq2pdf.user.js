...
// @namespace    wq2pdf
// @version      1.0.1
// @description  静默捕获文泉书局阅读页切片图片，CSV 记录变换方式，同步捕获目录写 <bid>_toc.json，左下角一键打包 zip；离线还原 PDF 请用配套 wq2pdf_reconstruct.py
 ...
 * ============================ 使用说明 ============================
 *
 * 1. 打开书籍阅读页后自动开始静默捕获，正常阅读滚动即可；
 *    想整本抓取就打开面板上的「自动滚动」开关。
 * 2. 切片图片来自 /deep/page/lmg/<bid>/<页码>?k=<一次性token>，
 *    脚本不发任何网络请求：
 *    - 优先 fetch(url, {cache:'only-if-cached'}) 只读浏览器 HTTP 缓存（无损）；
 *    - 失败则把已解码的 <img> 画到 canvas 转存 JPEG（q=0.95）。
 * 3. 点「打包下载」得到 <bid>.zip：
 *      3224451.csv      —— 每页一行：页码,变换方式
 *      img/p0055_01.jpg —— p+4位页码 + _ + 2位显示序（left 升序，1..N）
 *      3224451_toc.json —— 目录快照（与还原脚本缓存文件同名，解压即被优先采用）
 *      WARNING.txt      —— 仅在疑似限速占位图/缺片时出现
 * 4. 「清空缓存」后捕获会挂起（面板显示 ⏸），点 ▶ 或开启「自动滚动」才恢复
 *    ——避免清空后当前视口立刻被重新抓回；暂停期间手动滚动不会自动恢复。
 *    页面打开即同步抓取目录（catatree 接口），失败会在打包时再试一次；
 *    **暂停期间不主动抓目录**（含切走的自动暂停），恢复捕获后自动补抓；
 *    但暂停中手动「打包下载」属于主动操作，仍会抓取目录（保证 zip 带书签数据）。
 * 5. 面板两行布局：上行左侧 = 暂停/继续按钮（运行中为圆角方形 ⏸，
 *    暂停时为圆形 ▶，点击切换），右侧 = 打包下载；下行 = 清空缓存 / 自动滚动。
 *    任何暂停（手动 / 清空缓存）都需点 ▶ 或开启自动滚动恢复。
 * 6. 地址栏 bid 变化即视为另一本书（每秒检测一次，适配 SPA 换书）：记录、
 *    目录、打包归属都切到新书；旧书记录保留在缓存中，可切回继续。
 * 7. 多标签页：**只有最后激活的前台标签页在捕获**（localStorage 心跳锁，2s 续期）。
 *    切到哪个标签页，它就立即接管（focus / visibilitychange 即时刷新，不等心跳）；
 *    被顶掉的标签页自动暂停（显示 ⏸），切回时自动恢复（手动暂停/清空缓存后的暂停
 *    不受影响，仍须点 ▶）。非主标签页显示「⏳ 另有标签页正在捕获，本页待命（点此
 *    接管）」，点它或点 ▶ 也可手动接管。各书数据按 bid 严格隔离（IndexedDB 查询带
 *    KeyRange），不会跨书混入统计/打包/清空。
 * 8. 「清空缓存」默认只清当前书（有缓存时弹窗确认）；**当前书没有任何缓存时点它
 *    不会立刻清空**：第一次点击按钮变为「⚠ 再次点击清除全部缓存」且颜色加深，
 *    第二次点击才真正清空全部书籍的切片与目录快照；第一次点击后鼠标移开按钮即
 *    恢复原状（防误触）。清完自动重抓当前书目录。
 *
 * 脚本只负责捕获与打包；离线还原带目录书签的 PDF 请用配套
 * wq2pdf_reconstruct.py（同款拼图/旋转/书签流水线，无需浏览器）。
 *
 * 文件名后缀 = 显示序（left 升序），重组时可按变换方式自行换算：
 *
 * 变换规则（实测全站仅两种）：
 *   matrix(-1, 0, 0, -1, 0, 0)  原图上下颠倒 + 纵向拆分，left:0 = 原图最右一片
 *   matrix(0, -1, 1, 0, 0, 0)   原图横向拆分后顺时针旋转90°，left:0 = 原图最下一片
 *   ⇒ 按显示序（left 升序）拼合后整图旋转即可还原：
 *      前者转 180°，后者逆时针转 90°
 *
 * 仅供本人已合法获得访问权限的内容做离线备份，请勿传播产物。
 * ================================================================
 */
